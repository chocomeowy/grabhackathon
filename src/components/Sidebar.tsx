  ...
  return (
    <div className="relative h-full flex">
      ...
      <motion.aside 
        ...
        className="h-full glass-sidebar flex flex-col overflow-hidden relative shadow-2xl"
      >
        {/* VIBRANT GREEN Header */}
        <div className="p-10 pb-8 shrink-0 bg-primary/20 backdrop-blur-xl border-b border-white/10">
          <div className="flex items-center gap-4 mb-10">
            <div className="w-14 h-14 bg-primary rounded-2xl flex items-center justify-center text-white shadow-[0_8px_30px_rgba(0,177,79,0.3)] -rotate-3 border border-white/20">
            ...
            <div>
              ...
              <p className="text-[9px] font-black text-white/40 uppercase tracking-[0.4em]">Urban Intelligence Engine</p>
            </div>
          </div>

          <div className="space-y-6">
            <form onSubmit={(e) => { e.preventDefault(); handleSearch(); }} className="vibe-input-container relative flex items-center h-16 px-5 group">
              <Search className={`w-5 h-5 transition-colors ${isSearching ? 'text-primary' : 'text-muted-foreground'}`} />
              <input 
                type="text" 
                placeholder="Where's the vibe today?" 
                className="vibe-input flex-1 h-full ml-4 text-base"
                value={query}
              ...
              <button 
                ...
                className="ml-2 p-2.5 bg-primary rounded-xl text-white shadow-lg hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
              >
                {isSearching ? <Loader2 className="w-5 h-5 animate-spin" /> : <ChevronRight className="w-5 h-5" />}
              </button>
            </form>

            {/* Template Choices - Well Spaced */}
            <div className="flex flex-wrap gap-3">
               {TEMPLATES.map((t) => (
                 <button 
                   key={t.label}
                   onClick={() => handleSearch(t.query)}
                   className="px-5 py-2.5 bg-white/5 hover:bg-primary/20 border border-white/10 hover:border-primary/40 rounded-xl text-[10px] font-black text-white/80 hover:text-white transition-all whitespace-nowrap flex items-center gap-2 group"
                 >
                   <Zap className="w-3.5 h-3.5 text-primary group-hover:scale-110 transition-transform" /> {t.label}
                 </button>
               ))}
            </div>
          </div>
        ...
        {/* Content Scroll Area */}
        <div className="flex-1 overflow-y-auto px-10 pb-20 custom-scrollbar space-y-8">
